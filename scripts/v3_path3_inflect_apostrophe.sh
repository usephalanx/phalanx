#!/usr/bin/env bash
# Phase 3 canary — humanize inflect single-apostrophe regex fix re-derivation.
#
...
LOCAL_DIR="${LOCAL_DIR:-/tmp/inflect-regress}"
TARGET_COMMIT="${TARGET_COMMIT:-498619bf}"
TARGET_FILE="${TARGET_FILE:-inflect/__init__.py}"
RUN_ID="$(date -u +%Y%m%d-%H%M%S)"
BRANCH="path3/inflect-apostrophe-revert-${RUN_ID}"
...
(
  ...
  git -c user.name="record-bot" -c user.email="bot@phalanx.local" \
      commit -m "path3: revert ${TARGET_COMMIT}'s ${TARGET_FILE} to test v3 re-derivation" --quiet
...
PR_RESP=$(curl -s -X POST -H "Authorization: Bearer $GH_TOKEN" \
  ...
  -d "{\"title\":\"path3: revert inflect apostrophe-regex fix to test v3 re-derivation [$RUN_ID]\",\"head\":\"$BRANCH\",\"base\":\"main\",\"body\":\"Phalanx v3 wild-bug proof. Revert of $TARGET_COMMIT (src only). Bot will close after observation.\"}")
...
# Path 3 specific: GitHub disables Actions on fork PRs by default.
# Workaround: manually dispatch the workflow on the PR branch so CI
# actually runs. Resulting check_run events flow through the webhook
# normally and Phalanx v3 dispatches as on humanize.
WORKFLOW_ID=$(curl -s -H "Authorization: Bearer $GH_TOKEN" \
  "https://api.github.com/repos/$REPO/actions/workflows" \
  | python3 -c "import json,sys; d=json.load(sys.stdin); print(d['workflows'][0]['id'])" 2>/dev/null)
if [ -n "$WORKFLOW_ID" ]; then
  echo "  [setup] manually dispatching workflow $WORKFLOW_ID on $BRANCH"
  curl -s -X POST -H "Authorization: Bearer $GH_TOKEN" \
    -H "Accept: application/vnd.github+json" \
    "https://api.github.com/repos/$REPO/actions/workflows/$WORKFLOW_ID/dispatches" \
    -d "{\"ref\":\"$BRANCH\"}" -w "  [setup] dispatch HTTP %{http_code}\n"
fi
...
echo "inflect-path3|$final|$v3_run_id|$INTRO_SHA|${HEAD_SHA:-}"
