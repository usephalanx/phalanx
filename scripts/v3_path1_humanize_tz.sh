#!/usr/bin/env bash
# Phase 3 canary — humanize tz-aware datetime fix re-derivation.
#
...
TARGET_COMMIT="${TARGET_COMMIT:-a47a89e}"
TARGET_FILE="${TARGET_FILE:-src/humanize/time.py}"
RUN_ID="$(date -u +%Y%m%d-%H%M%S)"
BRANCH="path1/tz-revert-${RUN_ID}"
...
(
  ...
  git -c user.name="record-bot" -c user.email="bot@phalanx.local" \
      commit -m "path1: revert ${TARGET_COMMIT}'s ${TARGET_FILE} to test v3 re-derivation" --quiet
...
PR_RESP=$(curl -s -X POST -H "Authorization: Bearer $GH_TOKEN" \
  ...
  -d "{\"title\":\"path1: revert tz-fix to test v3 re-derivation [$RUN_ID]\",\"head\":\"$BRANCH\",\"base\":\"main\",\"body\":\"Phalanx v3 wild-bug proof. Revert of $TARGET_COMMIT (src only). Bot will close after observation.\"}")
