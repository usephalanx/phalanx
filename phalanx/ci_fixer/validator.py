...
def validate_fix(
    ...
    sandbox_result: object | None = None,
) -> ValidationResult:
    """
    Re-run the failing tool against the workspace to confirm the fix.

    When sandbox_result is provided and env_ready=True, runs validate_cmd
    inside the container (where deps are installed) instead of locally.
    ...
    # If sandbox is ready, run targeted validation inside the container
    if (
        sandbox_result is not None
        and getattr(sandbox_result, "env_ready", False)
        and getattr(sandbox_result, "container_id", "")
    ):
...
def _run_in_sandbox(
    ...
    original_parsed: object | None = None,
) -> ValidationResult:
    """
    Validate inside the provisioned container using targeted file-level checks.

    Strategy (avoids failing on pre-existing codebase-wide errors):
      1. Run the tool only on the patched files → must be clean.
      2. Regression check: run on patched files with original baseline,
         confirm no NEW errors were introduced.

    This is correct because:
      - A repo with 1000 pre-existing errors should not block a targeted fix.
      - We only need to prove: (a) the original error is gone, (b) no new errors.
    """
    ...
    targets = " ".join(patched_files) if patched_files else "."

    # Build targeted tool command inside the container
    if tool == "mypy":
        cmd = f"mypy --ignore-missing-imports --follow-imports=skip {targets}"
    elif tool == "ruff":
        cmd = f"ruff check {targets}"
    else:
        # Fallback: run the full validate_cmd
        cmd = getattr(sandbox_result, "validate_cmd", "") or f"{tool} {targets}"

    log.info(
        "ci_validator.sandbox_run",
        container_id=container_id,
        cmd=cmd,
        tool=tool,
        targets=patched_files,
    )

    code, output = _run_in_container(docker_cmd, container_id, cmd)

    # Parse errors from targeted run
    from phalanx.ci_fixer.log_parser import parse_log  # noqa: PLC0415, F811
    new_parsed = parse_log(output)

    # Key invariant: the original CI errors (by file + line) must be gone.
    # We don't flag pre-existing errors in the file that weren't in the CI log —
    # those existed before our patch and are not our responsibility.
    original_locations: set[tuple[str, int]] = set()
    if original_parsed is not None:
        for le in getattr(original_parsed, "lint_errors", []):
            original_locations.add((le.file, le.line))
        for te in getattr(original_parsed, "type_errors", []):
            original_locations.add((te.file, te.line))

    # Current errors on those exact lines = fix didn't work
    regressions = []
    for te in new_parsed.type_errors:
        if (te.file, te.line) in original_locations:
            regressions.append(te)
    for le in new_parsed.lint_errors:
    ...
    return ValidationResult(passed=True, tool=tool, output=output, tool_version=tool_version)


def _run_in_container(docker_cmd: str, container_id: str, shell_cmd: str) -> tuple[int, str]:
    """Run a shell command inside a Docker container, return (returncode, output)."""
    try:
        result = subprocess.run(
            [docker_cmd, "exec", "--workdir", "/workspace", container_id, "sh", "-c", shell_cmd],
            capture_output=True,
            text=True,
        ...
        return 1, f"(sandbox validation timed out after {_VALIDATE_TIMEOUT}s)"
    except Exception as exc:
        return 1, f"(sandbox validation error: {exc})"
