    ...
    async def _ask_gpt_env_setup(
        ...
        try:
            ...
            system_prompt = (
                ...
                "MINIMUM ordered shell commands needed to install dependencies "
                f"inside a bare {stack} container so the failing tool runs with the "
                "SAME versions CI uses. Version parity between sandbox and CI is "
                "critical: if CI installs ruff 0.15 and the sandbox has ruff 0.4, "
                "the sandbox will report 'pass' on code that CI rejects.\n\n"
                "CORE PRINCIPLE — mirror CI exactly:\n"
                "- Start by reading the failing job's install steps in the CI YAML "
                "and reproduce them verbatim (minus checkout/cache/upload steps). "
                "If CI runs `pip install -e \".[dev]\"`, you should too — that's "
                "how CI gets its pinned versions; the sandbox must get the same.\n"
                "- If the project's pyproject.toml / requirements*.txt pins a "
                "version of the failing tool (ruff, mypy, pytest, eslint, etc.), "
                "install that exact version so the sandbox behaves identically.\n"
                "- The repo source is ALREADY present at /workspace. `pip install "
                "-e .` is fine (it just registers the package); DO install the "
                "project's dev extras if that's how CI gets the lint tools.\n"
                "- Do NOT include checkout, git clone, git submodule, or any git "
                "commands — the workspace is already mounted.\n"
                "- Do NOT include cache steps, upload-artifact steps, or "
                "actions/checkout.\n"
                "- Do NOT include test execution or build commands. Only install "
                "+ the validate command.\n"
                "- Set validate_cmd to the exact command CI uses to run the "
                "failing tool (e.g. 'ruff check .', 'pytest tests/').\n"
                "- Working directory inside the container is /workspace.\n"
                "- Prefer `pip install --upgrade pip` before other pip installs "
                "only if CI does (don't add extra steps CI didn't have).\n\n"
