...
class SecurityAgent(BaseAgent):
    ...
    async def _run_security_pipeline(self, workspace: Path, run: Run) -> dict:
        ...
        """
        try:
            from phalanx.guardrails.security_pipeline import SecurityPipeline  # noqa: PLC0415
...
@celery_app.task(
    name="phalanx.agents.security.execute_task",
    bind=True,
    queue="security",
    max_retries=2,
    acks_late=True,
...
def execute_task(  # pragma: no cover
    ...
    try:
        ...
        log.exception("security.celery_task_unhandled", task_id=task_id, run_id=run_id)
